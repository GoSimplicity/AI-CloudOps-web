      ...
      >
        ...
        <template #labels="{ text }">
          <div class="k8s-labels-display">
            <a-tag v-for="label in (Array.isArray(text) ? text : []).slice(0, 3)" :key="label.key" class="k8s-label-item">
              {{ label.key }}: {{ label.value }}
            </a-tag>
            <a-tooltip v-if="(Array.isArray(text) ? text : []).length > 3" :title="(Array.isArray(text) ? text : []).map((item: any) => `${item.key}: ${item.value}`).join('\n')">
              <a-tag class="k8s-label-item">
                {{ (Array.isArray(text) ? text : []).length }} 个标签
              </a-tag>
            </a-tooltip>
            <span v-if="!text || !Array.isArray(text) || text.length === 0" class="k8s-no-data">-</span>
          </div>
      ...
      >
        ...
        <a-form-item label="容器镜像">
          <div class="k8s-key-value-inputs">
            <div v-for="(image, index) in createFormModel.images" :key="index" class="k8s-key-value-row">
              <a-input 
          ...
          <a-row :gutter="[24, 16]" style="margin-top: 16px;">
            <a-col :xs="24" :lg="12">
              <a-card title="标签信息" class="k8s-detail-card" size="small">
                <div class="k8s-labels-display">
                  <div v-for="label in (currentDeploymentDetail.labels || [])" :key="label.key" class="k8s-label-item" style="margin-bottom: 8px;">
                    {{ label.key }}: {{ label.value }}
                  </div>
            ...
            <a-col :xs="24" :lg="12">
              <a-card title="注解信息" class="k8s-detail-card" size="small">
                <div class="k8s-annotations-display">
                  <div v-for="annotation in (currentDeploymentDetail.annotations || [])" :key="annotation.key" class="k8s-annotation-item" style="margin-bottom: 8px;">
